...
### 2019/4/5 星期五
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;经过了前几天的学习铺垫，今天终于完成了对EM算法的学习总结，EM算法的学习需要一定的概率论知识（联合概率、条件概率、期望、对数似然函数）以及Jensen不等式，以上概念都不难，花点时间了解了即可。
 
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;EM算法，一句话说，就是根据已有的观测数据，初始话参数向量θ（θ包括了观测变量的概率和隐变量的概率！），然后不断迭代E步、M步，直至参数θ收敛的过程。
 
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;具体说一下EM算法的E步和M步吧，E即expectation（期望），我们把每一轮更新后的θ当做已知量，需要求出在给定参数θ和观测数据yi的前提下，隐变量zj的概率，即P(zj|yi;θ)。怎么求呢？举个例子，如果Z的取值有{z1,z2,z3}三种取值，由于θj是上一轮的迭代结果，所有在本轮中是已知的，于是这里就可以计算出P(z1，yi;θj)、P(z2，yi;θj)和P(z3，yi;θj)的值，然后由P(z1|yi;θj) = P(z1，yi;θj) / (P(z1，yi;θj)+P(z2，yi;θj)+P(z3，yi;θj))，同理，我们可以计算出另外的P(z2|yi;θj)和P(z3|yi;θj)，它们实际上就是我们在学习过程中定义的Qi(zi)。
 
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在本轮所有要计算出的Qi(zi)求解完之后，本轮的θj任务就完成了，可以更新我们的对数似然函数（关于θ的一个函数），E步结束，M步开始，就是计算对数似然函数最大时对应的θj+1，如果θj+1没有收敛的话，那么继续下一轮的迭代过程，以上就是EM算法的核心思路！

### 2019/4/1 星期一 

 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 之前的集成学习，暂时告一段落了，今天学习的任务是K-Means算法，这个算法在思想上还是比较简单的，但是实现起来比想象中复杂一点。首先得定义一些辅助函数，一是随机生成k个聚类中心，这个聚类中心不是从样本中选择的，而是在每一列特征的取值范围(min,max)随机生成k个值，对每一列进行这个操作，最终得到一个(k,n_features)的聚类中心矩阵centroids；二是定义一个计算两个向量欧氏距离的函数calc_euclidean，也很简单，向量相减求平方，再求和开根号.
 
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;说完了辅助函数之后，就来说说主函数kmeans了，输入值为特征矩阵，k值，输出聚类中心矩阵，样本所属簇的向量，定义一个布尔变量，标记聚类中心是否发生了变化，然后while循环中，遍历每一个样本点，对于每一个样本点再遍历与每一个聚类中心的距离，记录最小距离和对应的下标，对所有样本点遍历完之后，再重新计算聚类中心矩阵，也是一个遍历过程，首先找到当前聚类中心的所有样本点，cluster_samples = data_mat[samples_assignments==i]，然后再用centroids[i] = np.mean(cluster_samples,axis=0)，表示这个新的聚类中心。直至聚类中心不再发生改变为止

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;最后想聊一下k-means和knn的区别，虽然两个是完全不同的算法，但是之前老是容易混淆，首先一个是非监督学习算法，一个是监督学习算法，然后前者需要训练，以及指定k，将特征空间划分为k个簇，而后者只需要计算测试样本点与所有训练样本点的距离，然后选前k个样本，以多数投票法则，来预测样本点的类别.二者唯一的相似点，是都用了最近邻思想（nearest neighbor）. 

### 2019/3/28 星期四

 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;今天效率挺不错的，把xgboost的原理基本上了解完了，昨天学习了xgboost目标函数中损失函数的推导式,那么今天的重点其实就是正则项的推导了，主要是要约束新生成的树的模型。于是回顾了一下知识点，关于CART回归树的构造过程，这里是之前遗漏的知识点，CART回归树的表达式是由树中每个节点的最佳拟合值构成，输入一个样本x，把x从根节点到对应路径下的叶节点上的每个分值相加，即是该棵树的预测结果，但是XGBoost在树结构上的定义有了一些新的变化，那就是重新计算叶子的分值，说白了就是把从根节点到每个叶子节点上的分值相加，赋给叶子节点，然后在定义一个mapping函数，把输入的样本映射到叶子节点上，于是可以在新定义的树模型上，设定正则项了，然后回到最早的目标函数，将原本的训练节点，按照叶子节点进行分组，合并在一起计算，整合成一个新的二次函数式，根据二次函数的最小值定义，我们就可以找到最小的目标函数值了。在具体构建树的过程中，xgboost采用了贪心策略，每次选择最佳分裂节点是Gain最大时的节点，这个时候用到了并行化计算的技术，另外在针对特征值比较多的特征的分裂选择时，xgboost用到了weighted quantile sketch算法（看不下去了这里），在构建树时有两种技巧，一个是Pre-stopping,一个是Post-pruning，基本就介绍这么多了，xgboost在gdbt的算法上做了大量的改进，突出一点的是泰勒的二阶展开代替原损失函数，二是重新定义的树的结构，增加了树的正则项，之后就是在稀疏矩阵、缺失值、并行计算等方面做了大幅度的优化工作，真的是大佬了。

### 2019/3/27 星期三 

 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 今天虽然加班加的比较晚，到了晚上十点，但是主要还是因为白天效率太低了，浪费了不少时间，不过好在晚上的收获还是挺不错的，先来总结一下.
 
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在前几天学习了adaboost、rf、gdbt的基础下，终于有自信来学习慕名已久的xgboost啦！今天没学习完，大概看了一半，对于xgboost有了一个初步的认识。今天看的主要是xgboost以CART回归树作为基学习器的推导过程，跟着陈天齐大神的PPT走，更进一步地了解了机器学习的本质，那就是目标函数 = 损失函数 + 正则项 ，真的，这个概念之前是断断续续地学习，今天从整体上梳理了一下，感觉对于linear model了解的更透彻了.所以xgboost的学习也是按照这个步骤走，由于它也是addictive learning model，所以我们还是按照这个思想，来思考每一轮新增加的树，应该是什么样子的.在这里呢，一个核心的思想就是用泰勒二阶展开式对损失函数作近似估计，然后就可以把第t轮的目标函数转化的十分简洁了，今天暂时就学习到了这里，明天继续!
