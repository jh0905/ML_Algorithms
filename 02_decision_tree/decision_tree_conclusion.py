...
# ID3 算法的作者昆兰基于上述不足，对ID3算法做了改进，这就是C4.5算法，也许你会问，为什么不叫ID4，ID5之类的名字呢?
# 那是因为决策树太火爆，他的ID3一出来，别人二次创新，很快 就占了ID4， ID5，所以他另辟蹊径，取名C4.0算法，后来的进化版为C4.5算法
...
"""
 C4.5的改进: 针对上述的四个不足，分别作出改进，进化成C4.5 (推荐观看“西瓜书”《机器学习》周志华：P83 4.4连续值与缺失值章节)
    1.问题：不能处理连续型特征
        ...
        一共取得n-1个划分点，然后分别计算每一个划分点作为二元分类点时的信息增益，选择信息增益最大的点at作为该连续特征的二元离散分类点，
        小于at的值，为类别1，大于at的值为类别2，这就是连续特征离散化的过程；
