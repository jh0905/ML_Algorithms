...
"""
 1.SVM基本概念
    ...
    **而我们在前面知道，functional_margin与w,b是成比例变化的，它们所对应的分类超平面，仍然是同一个超平面，知道这一点之后，为了简化计算，
    ...
    (7) 根据SVM的对偶问题的解α*=(α1*,α2*,...,αN*).T，如何计算原始问题的解w*,b* 
        我们在解对偶问题时，首先是把w,b求偏导，令其值为0，得到了：
                                w = sum αi*yi*xi        , i=1,2,...,N ,
                                
        于是根据上式，我们代入α*=(α1*,α2*,...,αN*).T， 可求出
                                w* = sum (αi*)*yi*xi            ，其中i=1,2,3,...,N
          
        对于b，我们知道，所有支持向量(xs,ys)，都满足：
                                ys(w.T*xs+b) = 1 即 ys * (sum (αi*)*yi*(xi∙xs) + b) = 1   i=1,2,...,N
           
        理论上，对于任意一个支持向量，我们都能求出一个b*，结果也应该是相同的，但是为了算法的健壮性，这里的做法是把所有支持向量都代入到上式,
    最终再取平均值，作为最终的b*.
        
        对于某个支持向量(xs,ys)，将 ys * (sum (αi*)*yi*(xi∙xs) + b) = 1 等式两边同乘以ys，得：
                                b* =  ys - sum (αi*)*yi*(xi∙xs)   【如果有k个支持向量的话，计算出k个b*,然后求均值】
                                
        于是我们最终的分类超平面为 ： (w*).T * x + b* = 0  ,  SVM的分类判别函数 y = sign((w*).T * x + b*) 
 ...
 3.软间隔最大化时的支持向量##  【这一小节超级重要，对后面理解SMO算法的一些地方很有用!】
    
    “硬间隔的支持向量，理解为那些落在间隔边界上的点，即当αi* > 0时对应的点；
    软间隔的支持向量，理解为那些落在间隔边界上或者在间隔边界与分类超平面之间或者误分类的点，即 0 < αi* <= C时对应的点”
      
    可以换个角度来想，我们可以认为支持向量，是那些对目标函数起决策性作用的样本点，换言之，除去远离间隔边界的正确分类的样本点，剩下的样本点
    都是支持向量.
    ...
    根据软间隔最大化时KKT的对偶互补条件，(αi*) * (yi(w.T*xi+b) + ξi - 1) = 0
        ...
        如果 αi* = C , 说明这可能是一个异常的点，需要检查此时的ξi
            (a) 0 <= ξi < 1   ，样本点被正确分类，但是在超平面和自己类别的间隔之间；
            (b) ξi = 1        ，样本点在分类超平面上，无法正确分类；
            (c) ξi > 1        ，样本点在超平面的另一侧，不能被正常分类(可能是噪音点)；     
